...
- **Scoping a query invalidates every cache-shape assumption downstream of it.** When hydration was scoped to the user's memberships, `dedupePublished` kept validating published rows' `tripId`s against `cache.trips` — which now only holds *your* trips, so the "orphan" filter silently dropped every other creator's publication and emptied Explore. Before changing what a fetch returns, grep for consumers that derive invariants (valid-id sets, orphan checks, existence filters) from that data, and fix or remove them in the same change.
- **A "re-hydrate at the end" that can itself re-trigger the first operation is an unbounded loop** (`seedDemoFor` → hydrate → 0 trips → seed). If a function's success path re-runs hydration and hydration re-runs that function on empty data, pass an explicit "don't re-seed / don't recurse" flag down the second call. Also: never seed from a hydration whose Stage-1 selects errored — a flaky `trip_members` read must abort with a toast, not look like a fresh account and mint duplicate demo trips.
