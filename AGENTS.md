...
- **View Transitions + theme radiate (Sep 2026): `fill: 'both'` cures the end-of-transition flash, but do NOT "fix" the pre-ready gap by pinning the start state in CSS** — adding a `::view-transition-new(root)` clip class before `vt.ready` made dark→light render wrong (full-cream fill) where the plain JS `animate()` was already correct. Lesson: this transition's pre-ready gap is invisible in practice; only add plumbing when a symptom is actually observed, and restore the last-known-good iteration verbatim when a "fix" regresses feel (`4ac890a` → `f918a2e`). See `toggleTheme` in `src/App.tsx`.
