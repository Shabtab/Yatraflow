...
- **View Transitions: pin the start state in CSS *before* `startViewTransition`** — the new snapshot renders **unclipped** for the frames between capture and `vt.ready` (where JS `animate()` attaches), so a circle-reveal flashes as a full-screen fill of the incoming theme (theme-toggle radiate, Sep 2026). Pattern: set origin coords as CSS vars on `:root` pre-transition, add a class whose rule clips the pseudo (`:root.theme-vt-expand::view-transition-new(root) { clip-path: circle(0px at var(--vt-x) var(--vt-y)) }`), animate with `fill: 'both'` so the end-state also holds until teardown, remove the class on `vt.finished`. See `toggleTheme` in `src/App.tsx`.
