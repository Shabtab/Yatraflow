...
- **View Transitions + theme radiate (Sep 2026): don't use VT snapshots for the theme toggle — `backdrop-filter` glass renders as a flat gray veil inside snapshots.** The dark theme's full-width glass (`rgba(255,255,255,.58)`) lost its backdrop in the captured page and the whole screen read as mid-gray during the radiate (page-dependent — looked "perfect" on Landing, broken on #/trips). The shipped solution is a plain DOM overlay (`.theme-radiate`, light-canvas radial gradient anchored at the button via `--vt-x/--vt-y`): expands `circle(0→r)` for dark→light (swap theme at cover, then fade overlay into the real UI), collapses `circle(r→0)` for light→dark (swap immediately). No snapshot artifacts, works everywhere. Restoring last-known-good iterations verbatim when a "fix" regresses feel (`4ac890a` → `f918a2e`) was also learned the hard way here. See `toggleTheme` in `src/App.tsx`.
