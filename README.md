...
- **Board view — spatial planning over the route map** — a kanban-style Board tab (`#/trip/<id>/board`) pins the live map as the canvas with near-opaque day columns floating over it: drag stops between days or reorder within a day (desktop drag + mobile long-press), watch the teal insertion marker pick the landing slot and everything settle with a single FLIP animation, focus the map on any day's route, or slide the columns away with map-focus peek mode for unhurried route reading. A Trip Pulse panel keeps health, decisions and budget in view; every drop routes through the same impact-preview flow, and staged changes are visible identically from every tab
...
### Share
- **Publish itineraries** to the public **Explore** gallery with tagline, best season, travel tips and warnings — an editorial discovery surface with route-aware search, travel-style chips, featured itineraries and per-card credibility (forks · views · trip health)
- Readers can **save favourites** (device-local, no account needed) and **copy any published trip** into their own plans in one click and make it theirs
