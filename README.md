...
- **Long-ride break planner** — a long self-drive day (e.g. ~580 km Kolkata → Siliguri) isn't offered as one sitting. Pick how aggressive the ride is — **push through / one halt / regular breaks** (a stretch every ~2.5h, closer on a motorcycle) — set the day's **ride start time**, and see total ride time and the arrival clock shift as you add halts. Each recommended halt states where along the route (km + % in), how long (meal vs tea/fuel stretch) and the rough clock time; you can **suggest real stop spots** along that stretch of the corridor (food/fuel/rest from open map sources) and add them through the normal impact preview. A fatigue warning flags 7h+ wheel-time days with no halt. The planner also surfaces on a self-drive **outbound departure day** (a day holding only the trip-start anchor plans the drive to the next destination), so Kolkata → Siliguri shows it on both the way out and the way back.
