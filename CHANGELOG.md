...
### Fixed
- **Preview builds could not log in — a missing build-time env is now loud instead of cryptic** — Vercel's `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` were scoped to **Production only**, so every Preview deploy was compiled with no backend: `src/lib/supabase.ts` fell back to its deliberate placeholder client (`http://localhost:54321`), the app rendered normally, and login died with a bare "Failed to fetch" that reads exactly like a wrong password. Confirmed by comparing shipped bundles: production's `index-*.js` contains a real `https://…supabase.co` host, a preview's contains none. Fixed in three layers: (1) `vite.config.ts` now **aborts the build on Vercel** when either variable is absent or still the `YOUR-PROJECT` template, naming the environment (`preview`/`production`) and the remedy — CI and fresh clones get a warning only, since they legitimately build without credentials; (2) new pure `src/lib/authErrors.ts` (`isRealSupabaseUrl` / `describeAuthFailure` / `MISSING_BACKEND_MESSAGE`) lets `store.login`/`signup` short-circuit with the actual cause, and catches Supabase's **thrown** `AuthRetryableFetchError` — previously an unhandled rejection that left the form silently re-enabling after its 10 s failsafe with no message at all; (3) the auth page now shows a persistent "no backend configured" banner before the first attempt. Also `login()` lowercases the email to match `signup()`, so an address typed with capitals at signup can't lock its owner out. 8 new tests in `tests/auth-errors.test.ts`. **Action required:** tick both variables for Preview (and Development) under Vercel → Settings → Environment Variables, then **Redeploy** — editing them never affects an already-built deployment, because Vite inlines at build time.
