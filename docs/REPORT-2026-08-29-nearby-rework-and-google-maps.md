...
### Scope — only the three data pieces Google does best for us
We use Google **only for data**: (1) place autocomplete with coordinates for location picking, (2) nearby POI data fed into our tourist engine, and (3) **opening hours** — requested via FieldMask (`places.regularOpeningHours` + `places.currentOpeningHours`) on the **same** Text Search events, so they cost **zero extra SKU events** and need no Place Details calls. Routing stays on OSRM, weather on Open-Meteo, rendering on MapLibre; hours fall back to Overpass in free mode. No Directions API, no Google map rewrite.
...
- **Phase A — provider facade (~1 d):** `src/lib/providers/free.ts` (today's engines, untouched) + `src/lib/providers/google.ts` (Places Autocomplete for location picking; Text Search with `searchAlongRouteParameters` for nearby ideas — FieldMask includes `regularOpeningHours`/`currentOpeningHours`, so hours arrive on the same events). `geocode.ts` becomes a facade: Google first → free stack on key-absence, error, or quota exhaustion. All Phase 1–4 logic untouched. `resolveHitCoords`/Nominatim fallback deleted (Google returns coordinates).
...
**What we'd gain:** India-best autocomplete with coordinates for free; richer en-route POI data with native detour ranking; **reliable opening hours on suggestion cards** (Google's India hours coverage vastly exceeds OSM's sparse `opening_hours` tags — and "open now" comes free via `currentOpeningHours`); no CORS proxy for the Google path.
...
| Route legs / weather | 0 (OSRM, Open-Meteo — all free); hours ride on the same Text Search events |
| **Per trip** | **~11** |
...
| Stale/wrong hours | Google hours render as "reported" — fetched live per search, never persisted (ToS + staleness); Overpass hours remain the free-mode fallback |

---

## 6. Open questions for discussion

1. **Adopt Google now, or stay on the free stack?** The simulation says ₹0 with huge headroom (India SKUs verified); the main argument *for* is India-best autocomplete + native Search-Along-Route detour ranking + reliable opening hours — all on the same ~1.5-day build. The main argument *against* is the Places ToS gray zone on MapLibre display/blending.
